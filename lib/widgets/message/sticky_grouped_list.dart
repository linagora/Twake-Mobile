/// https://github.com/Dimibe/sticky_grouped_list/blob/master/lib/sticky_grouped_list.dart
import 'dart:async';
...
class _StickyGroupedListViewState<T, E>
  ...
  _positionListener() {
    ...
    if (_topElementIndex != index &&
        _topElementIndex < _sortedElements.length) {
  ...
  Widget _showFixedGroupHeader(int index) {
    if (widget.elements.length > 0) {
      ...
      return Container(
        ...
        child: index < _sortedElements.length
            ? widget.groupSeparatorBuilder(_sortedElements[index])
            : Container(),
      );
    }
    return Container();
